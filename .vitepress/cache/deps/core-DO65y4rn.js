import { Fragment, TransitionGroup, computed, customRef, defineComponent, effectScope, getCurrentInstance, getCurrentScope, h, hasInjectionContext, inject, isReactive, isReadonly, isRef, markRaw, nextTick, onBeforeMount, onBeforeUnmount, onBeforeUpdate, onMounted, onScopeDispose, onUnmounted, onUpdated, provide, reactive, readonly, ref, shallowReactive, shallowRef, toRaw, toRef, toRefs, toValue, unref, watch, watchEffect } from "./vue.runtime.esm-bundler-DP3eo4Mj.js";
...
function onClickOutside(target, handler, options = {}) {
	...
	if (isIOS && !_iOSWorkaround) {
		...
		Array.from(window$1.document.body.children).forEach((el) => useEventListener(el, "click", noop, listenerOptions));
		useEventListener(window$1.document.documentElement, "click", noop, listenerOptions);
	}
...
function onElementRemoval(target, callback, options = {}) {
	const { window: window$1 = defaultWindow, document: document$1 = window$1 == null ? void 0 : window$1.document, flush = "sync" } = options;
...
function useBluetooth(options) {
	...
	async function requestDevice() {
		...
		try {
			device.value = await (navigator$1 == null ? void 0 : navigator$1.bluetooth.requestDevice({
				acceptAllDevices,
				filters,
				optionalServices
			}));
...
function useStorage(key, defaults$1, storage, options = {}) {
	...
	if (window$1 && listenToStorageChanges) tryOnMounted(() => {
		if (storage instanceof Storage) useEventListener(window$1, "storage", update, { passive: true });
		else useEventListener(window$1, customStorageEventName, updateFromCustomEvent);
...
function useCssVar(prop, target, options = {}) {
	...
	function updateCssVar() {
		...
		if (el && window$1 && key) {
			const value = (_a = window$1.getComputedStyle(el).getPropertyValue(key)) == null ? void 0 : _a.trim();
...
function useDeviceMotion(options = {}) {
	...
	function init() {
		if (window$1) {
			...
			useEventListener(window$1, "devicemotion", onDeviceMotion, { passive: true });
		}
	}
...
function useDevicesList(options = {}) {
	...
	async function update() {
		...
		onUpdated$1 == null || onUpdated$1(devices.value);
		if (stream) {
...
function useDisplayMedia(options = {}) {
	...
	const isSupported = useSupported(() => {
		var _a2;
		return (_a2 = navigator$1 == null ? void 0 : navigator$1.mediaDevices) == null ? void 0 : _a2.getDisplayMedia;
	});
	const constraint = {
		audio,
		video
	...
	const applyIcon = (icon) => {
		...
		if (!elements || elements.length === 0) {
			...
			if (link) {
				...
				document$1 == null || document$1.head.append(link);
			}
			return;
		}
	...
	function setMethod(method) {
		return (payload, payloadType) => {
			if (!isFetching.value) {
				...
				if (isRef(config.payload)) watch([refetch, toRef$1(config.payload)], ([refetch2]) => refetch2 && execute(), { deep: true });
				return {
					...shell,
					then(onFulfilled, onRejected) {
...
function useFullscreen(target, options = {}) {
	...
	const targetRef = computed(() => {
		var _a;
		return (_a = unrefElement(target)) != null ? _a : document$1 == null ? void 0 : document$1.documentElement;
	});
...
function useGamepad(options = {}) {
	...
	const updateGamepadState = () => {
		const _gamepads = (navigator$1 == null ? void 0 : navigator$1.getGamepads()) || [];
...
function useIdle(timeout = oneMinute, options = {}) {
	...
	if (window$1) {
		...
		const listenerOptions = { passive: true };
		for (const event of events$1) useEventListener(window$1, event, onEvent, listenerOptions);
...
function useMouseInElement(target, options = {}) {
	...
	const targetRef = shallowRef(target != null ? target : window$1 == null ? void 0 : window$1.document.body);
...
function useNavigatorLanguage(options = {}) {
	...
	const language = shallowRef(navigator$1 == null ? void 0 : navigator$1.language);
...
function useScrollLock(element, initialState = false) {
	...
	const unlock = () => {
		const el = resolveElement(toValue(element));
		if (!el || !isLocked.value) return;
		if (isIOS) stopTouchMoveListener == null || stopTouchMoveListener();
...
function useTextareaAutosize(options = {}) {
	var _a, _b;
	const { window: window$1 = defaultWindow } = options;
	const textarea = toRef$1(options == null ? void 0 : options.element);
	const input = toRef$1((_a = options == null ? void 0 : options.input) != null ? _a : "");
...
function formatTimeAgo(from, options = {}, now$1 = Date.now()) {
	...
	for (const [idx, unit] of units.entries()) {
		const val = getValue$1(diff, unit);
...
function useTitle(newTitle = null, options = {}) {
	...
	const title = toRef$1((_b = newTitle != null ? newTitle : document$1 == null ? void 0 : document$1.title) != null ? _b : null);
	...
	watch(title, (newValue, oldValue) => {
		if (newValue !== oldValue && document$1) document$1.title = format(newValue != null ? newValue : "");
	}, { immediate: true });
...
function useUrlSearchParams(mode = "history", options = {}) {
	const { initialValue = {}, removeNullishValues = true, removeFalsyValues = false, write: enableWrite = true, writeMode = "replace", window: window$1 = defaultWindow } = options;
	...
	function write(params, shouldUpdate) {
		...
		else window$1.history.pushState(window$1.history.state, window$1.document.title, window$1.location.pathname + constructQuery(params));
		resume();
	}
	function onChanged() {
...
function useUserMedia(options = {}) {
	...
	const isSupported = useSupported(() => {
		var _a2;
		return (_a2 = navigator$1 == null ? void 0 : navigator$1.mediaDevices) == null ? void 0 : _a2.getUserMedia;
	});
	const stream = shallowRef();
...
function useWakeLock(options = {}) {
	...
	if (isSupported.value) {
		...
		whenever(() => documentVisibility.value === "visible" && (document$1 == null ? void 0 : document$1.visibilityState) === "visible" && requestedType.value, (type) => {
